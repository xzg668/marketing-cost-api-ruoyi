...
public class MakePartPriceCalcResolver implements PriceResolver {
  ...
  public PriceResolveResult resolve(String oaNo, CostRunPartItemDto item, PriceTypeRoute route) {
    ...
    String businessUnitType = trimToNull(BusinessUnitContext.getCurrentBusinessUnitType());
    String pricingMonth = YearMonth.now().toString();
    if (oaNoValue == null) {
    ...
    MakePartPriceCalcRow latest = selectLatestComplete(parentCode, oaNoValue, pricingMonth, businessUnitType);
    if (latest == null) {
    ...
    return new PriceResolveResult(
        ...
        "取自制造件价格生成结果：批次=" + latest.getCalcBatchId() + "，价格月份=" + pricingMonth);
  }

  private MakePartPriceCalcRow selectLatestComplete(
      String parentMaterialNo, String oaNo, String pricingMonth, String businessUnitType) {
    // 制造件价格可能受 OA 场景影响，不能回退其他 OA、空 OA 或通用批次。
    // 当前阶段没有月度调价模块，价格月份按系统当前月份取。

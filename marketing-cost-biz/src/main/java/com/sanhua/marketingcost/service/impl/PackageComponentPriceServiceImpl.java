...
public class PackageComponentPriceServiceImpl implements PackageComponentPriceService {
  ...
  public PackagePriceResult ensurePrice(PackagePriceRequest request) {
    ...
    if (!req.forceRefresh) {
      PackageComponentPrice reusable =
          selectReusableCompletePrice(req.periodMonth, req.packageMaterialCode, req.topProductCode);
      if (reusable != null) {
  ...
  private ResolvedChild resolveChild(
    ...
    for (PriceTypeRoute route : candidates) {
      ...
      PriceResolveResult resolved = resolver.resolve(req.oaNo, item, route);
  ...
  private PackageComponentPrice ensurePriceRow(NormalizedRequest req, PackageComponentSnapshot snapshot) {
    PackageComponentPrice existing =
        selectByPeriodPackageAndTop(req.periodMonth, req.packageMaterialCode, req.topProductCode);
    if (existing != null) {
  ...
  private PackageComponentPrice selectReusableCompletePrice(
      String periodMonth, String packageMaterialCode, String topProductCode) {
    PackageComponentPrice existing = selectByPeriodPackageAndTop(periodMonth, packageMaterialCode, topProductCode);
    if (existing == null) {
      return null;
    }
    ...
    return complete && priced && existing.getTotalPrice() != null ? existing : null;
  }

  private PackageComponentPrice selectByPeriodPackageAndTop(
      String periodMonth, String packageMaterialCode, String topProductCode) {
    return priceMapper.selectOne(
        Wrappers.<PackageComponentPrice>lambdaQuery()
            .eq(PackageComponentPrice::getPeriodMonth, periodMonth)
            .eq(PackageComponentPrice::getPackageMaterialCode, packageMaterialCode)
            .eq(PackageComponentPrice::getSourceTopProductCode, topProductCode)
            .last("LIMIT 1"));
  }
  ...
  private PackageSnapshotRequest toSnapshotRequest(NormalizedRequest req) {
    ...
    snapshotRequest.setAsOfDate(req.asOfDate);
  ...
  private NormalizedRequest normalize(PackagePriceRequest request) {
    ...
    LocalDate quoteDate = request.getAsOfDate() == null ? LocalDate.now() : request.getAsOfDate();

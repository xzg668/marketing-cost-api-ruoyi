...
import com.sanhua.marketingcost.service.technicaldata.TechnicalDataAuxiliaryHistoryRow;
...
public interface TechnicalDataAuxiliaryReferenceMapper {
  ...
  @Select("""
      ...
      SELECT source_product.id AS source_product_id,
             ...
             source_version.auxiliary_total_amount AS total_amount,
             (SELECT COUNT(*) FROM lp_quote_tech_aux_item item
       ...
       WHERE source_product.id&lt;&gt;#{targetProductId}
         ...
         AND EXISTS (SELECT 1 FROM lp_quote_tech_aux_item item
                      WHERE item.version_id=source_version.id)
         AND NOT EXISTS (
             SELECT 1 FROM lp_quote_tech_aux_item invalid_item
              WHERE invalid_item.version_id=source_version.id
                AND (TRIM(invalid_item.subject_code)=''
                  OR TRIM(invalid_item.auxiliary_material_no)=''
                  OR TRIM(invalid_item.auxiliary_name)=''
                  OR invalid_item.quantity&lt;=0
                  OR TRIM(invalid_item.original_unit)=''
                  OR invalid_item.reference_unit_price&lt;=0
                  OR TRIM(invalid_item.price_unit)=''
                  OR invalid_item.loss_rate&lt;0 OR invalid_item.loss_rate&gt;1))
      ...
      """)
  List<TechnicalDataAuxiliaryHistoryRow> selectApprovedSources(

-- =============================================================================
-- V73: 修复 CMS 回收废料映射表结构
-- -----------------------------------------------------------------------------
-- 部分开发库在 V69 初版已被 Flyway 标记执行，后续 V69 补充的 CMS 展示/追溯字段
-- 不会自动重跑，导致 MaterialScrapRef 实体查询 material_name 等字段时报 Unknown column。
-- 本迁移幂等补齐 lp_material_scrap_ref 与当前实体一致的字段。
-- =============================================================================

DROP PROCEDURE IF EXISTS _cms_material_scrap_repair_add_column_if_not_exists;
DROP PROCEDURE IF EXISTS _cms_material_scrap_repair_add_index_if_not_exists;

DELIMITER //

CREATE PROCEDURE _cms_material_scrap_repair_add_column_if_not_exists(
...
BEGIN
  IF NOT EXISTS (
    ...
    SET @cms_material_scrap_repair_add_column_sql =
      CONCAT('ALTER TABLE ', p_table_name, ' ', p_column_definition);
    PREPARE cms_material_scrap_repair_add_column_stmt
      FROM @cms_material_scrap_repair_add_column_sql;
    EXECUTE cms_material_scrap_repair_add_column_stmt;
    DEALLOCATE PREPARE cms_material_scrap_repair_add_column_stmt;
  END IF;
END //

CREATE PROCEDURE _cms_material_scrap_repair_add_index_if_not_exists(
  IN p_table_name VARCHAR(64),
...
BEGIN
  IF NOT EXISTS (
    ...
    SET @cms_material_scrap_repair_add_index_sql =
      CONCAT('ALTER TABLE ', p_table_name, ' ', p_index_definition);
    PREPARE cms_material_scrap_repair_add_index_stmt
      FROM @cms_material_scrap_repair_add_index_sql;
    EXECUTE cms_material_scrap_repair_add_index_stmt;
    DEALLOCATE PREPARE cms_material_scrap_repair_add_index_stmt;
  END IF;
END //
...
CALL _cms_material_scrap_repair_add_column_if_not_exists(
...
CALL _cms_material_scrap_repair_add_index_if_not_exists(
  'lp_material_scrap_ref',
